/*
	MODIFIED FROM: https://github.com/salviati/go-qrand
...
func (q *QRNG) Read(rand []byte) (int, error) {
	if len(rand) == 0 {
		return 0, nil
	}
	...
	if responseCode != 0 || remedyCode != 0 {
		...
		return 0, errors.New("QRNG: " + resp + ": " + rem)
	}
...
func (q *QRNG) ReadBytes(p []byte) (int, error) {
	q.Lock()
	defer q.Unlock()
	return io.ReadFull(q.buf, p)
}

func (q *QRNG) readInto(v interface{}) error {
	n := 0
	...
	case *uint64, *int64, *float64:
		n = 8
	default:
		return errors.New(fmt.Sprint("QRNG.readInto: unexpected data type ", t))
	}
...
func (q *QRNG) Float64() (float64, error) {
	var r uint64
	err := q.readInto(&r)
	return float64(r) / (1 << 64), err
}

// New creates a new instances of Quantum Random Bit Generator client.
// The client-side should have a username and password from the relevant web-site.
func New(user, pass string, buffersize int) (*QRNG, error) {
	if buffersize < 1 {
		return nil, errors.New("QRNG: buffersize is too small")
	}

	q := &QRNG{user: user, pass: pass}
